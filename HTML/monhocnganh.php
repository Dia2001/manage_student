        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="wrapper">
                    ...
                    <div>
                        <button class='btn btn-success' onclick="window.open('inputmonhocnganh.php','_self')"><span class='glyphicon glyphicon-plus'><i class="bi bi-plus-lg"></i>Thêm mới</button>
                    </div>
                </div>
                <?php
                $dong = 6;
                ...
                foreach ($list as $cla) {
                    echo '<tr>
                        ...
                        <td>' . $cla['tenmhn'] . '</td>
                        <td>' . $cla['manganh'] . '</td>
                        <td><button class="btn btn-warning" onclick=\'window.open("inputmonhocnganh.php?id=' . $cla['mamhn'] . '","_self")\'><i class="bi bi-brush"></i>Sửa</button></td>
                        <td><button class="btn btn-danger" onclick="deleteqs(\'' . $cla['mamhn'] . '\')"><i class="bi bi-trash"></i>Xóa</button></a></td>
                    </tr>';
                }
                echo "</tbody>";
                echo "<table>";
                ...
                for ($i = 0; $i <= $sotrangdl; $i++) {
                    $trangchon = $i + 1;
                    echo "<li class='list-group-item page-item'><a href='monhocnganh.php?page=$i'>$trangchon</a></li>";
                }
                echo "</ul>";
                ?>
                <script type="text/javascript">
                    function deleteqs(mmhn) {
                        console.log(mmhn)
                        a = confirm('Bạn có muốn xóa môn học này?')
                        if (!a) {
                            return;
                        }
                        $.post('deletemonhocnganh.php', {
                                'x': mmhn
                            },
                            function(data) {
                                location.reload()
